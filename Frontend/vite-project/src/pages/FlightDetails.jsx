...
const airlineData = {
  ...
  CX: {
    name: "Cathay Pacific",
    logo: "https://www.executivetraveller.com/photos/view/size:960,540/5449cfd87e0c47e6bf1055de767f2254-cx-new-look-logo-1500a.jpg",
  },
...
const FlightDetails = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const [selected, setSelected] = useState("yes"); // Trip Secure
  const [selectedFare, setSelectedFare] = useState("saver"); // Fare Option
  const flight = location.state?.flight;

  const [travellers, setTravellers] = useState([
    { id: 1, firstName: "", lastName: "", gender: "MALE", email: "", phone: "" },
  ]);
  ...
  const addTraveller = () => {
    setTravellers([
      ...travellers,
      { id: travellers.length + 1, firstName: "", lastName: "", gender: "MALE", email: "", phone: "" },
    ]);
  };
  ...
  {segments.map((seg, idx) => (
    <div
      ...
      className="bg-white shadow-md rounded-xl p-6 border border-gray-200"
    >
      {/* Top row: Departure - Timeline - Arrival */}
      <div className="flex items-center justify-between relative">
        {/* Departure */}
        <div className="w-1/3 text-left">
          <p className="text-2xl font-bold text-gray-800">
          ...
          <p className="text-sm text-gray-500">
            {getAirportName(seg.departure.iataCode)+" ,Terminal T2"}
          </p>
          <p className="text-md font-semibold text-blue-600 mt-1">
        ...
        <div className="w-1/3 text-right">
          ...
          <p className="text-sm text-gray-500">
            {getAirportName(seg.arrival.iataCode,)+" ,Terminal T1"}
          </p>
          <p className="text-md font-semibold text-green-600 mt-1">
      ...
      {/* ✅ Airline Info with Airline Logo & Class */}
      <div className="mt-4 flex justify-between items-center text-xs text-gray-600">
        <div className="flex items-center gap-2">
          <img
            src={airlineData[seg.carrierCode]?.logo}
          ...
          />
          <p className="text-sm font-semibold">
            {airlineData[seg.carrierCode]?.name} ({seg.carrierCode} {seg.number})
          </p>
        </div>
        <div className="text-sm font-semibold text-blue-600">
        ...
        <div className="bg-white shadow-md rounded-lg p-4 mb-6 mt-[20px]">
          ...
          {travellers.map((trav, idx) => (
            ...
            >
              {/* Adult Title with Remove Button */}
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-semibold text-blue-600">ADULT {idx + 1}</h3>
                {travellers.length > 1 && (
                  <button
                    onClick={() => removeTraveller(trav.id)}
                    className="text-red-500 text-sm font-medium hover:underline"
        ...
        <div className="bg-white shadow-md rounded-lg p-6 border border-gray-200 mt-6">
          <h2 className="text-lg font-bold text-gray-800 mb-4">Select Fare</h2>

          <div className="grid md:grid-cols-3 gap-4">
            {/* Saver */}
            <label className="border rounded-lg p-4 hover:shadow-md cursor-pointer flex flex-col">
              <div className="flex justify-between items-center">
                <h3 className="font-semibold text-blue-600 mb-2">Saver</h3>
                <input
                  type="radio"
                  name="fare"
                  value="saver"
                  checked={selectedFare === "saver"}
                  onChange={() => setSelectedFare("saver")}
                />
              </div>
            ...
            <label className="border rounded-lg p-4 hover:shadow-md cursor-pointer flex flex-col">
              <div className="flex justify-between items-center">
                <h3 className="font-semibold text-green-600 mb-2">Flexi Plus</h3>
                <input
                  type="radio"
                  name="fare"
                  value="flexi"
                  checked={selectedFare === "flexi"}
                  onChange={() => setSelectedFare("flexi")}
                />
              </div>
              <ul className="text-sm text-gray-600 space-y-1 mb-3">
                <li>✔ Cabin Bag: 7kg</li>
                ...
                <li>✔ Low Cancellation Fee</li>
              </ul>
              <p className="text-right font-bold text-gray-800">
                ₹ {Number(flight.price.total) + 800}
              </p>
            </label>
            ...
            <label className="border rounded-lg p-4 hover:shadow-md cursor-pointer flex flex-col">
              <div className="flex justify-between items-center">
                <h3 className="font-semibold text-purple-600 mb-2">MMT Special</h3>
                <input
                  type="radio"
                  name="fare"
                  value="mmt"
                  checked={selectedFare === "mmt"}
                  onChange={() => setSelectedFare("mmt")}
                />
              </div>
              <ul className="text-sm text-gray-600 space-y-1 mb-3">
                <li>✔ Cabin Bag: 7kg</li>
              ...
              <p className="text-right font-bold text-gray-800">
                ₹ {Number(flight.price.total) + 1500}
              </p>
            </label>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-center">
          <button
            onClick={() =>
              alert(
                `Travellers: ${JSON.stringify(travellers)} | Fare: ${selectedFare} | Secure: ${selected}`
              )
            }
            className="px-6 py-3 bg-green-600 text-white font-semibold rounded-lg shadow hover:bg-green-700"
          >
